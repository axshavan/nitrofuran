...
switch($_REQUEST['page'])
{
	...
	case 1:
	{
		...
		{
			$sql = "insert into `".KASSA_ACCOUNT_TABLE."` set `name` = '".$DB->EscapeString($_REQUEST['addaccount'])."', `default` = '".($_REQUEST['default'] ? 1 : 0)."', `show` = '".($_REQUEST['show'] ? 1 : 0)."', `warnlimit` = '".($_REQUEST['warnlimit'] ? 1 : 0)."'";
		}
		...
		{
			$sql = "update `".KASSA_ACCOUNT_TABLE."` set `name` = '".$DB->EscapeString($_REQUEST['name'])."', `default` = '".($_REQUEST['default'] ? 1 : 0)."', `show` = '".($_REQUEST['show'] ? 1 : 0)."', `warnlimit` = '".($_REQUEST['warnlimit'] ? 1 : 0)."' where `id` = '".(int)$_REQUEST['editaccount']."'";
		}
