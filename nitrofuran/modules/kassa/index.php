...
// в кассе всего
$_sum_all = array();
$res = $DB->Query("select sum(k.`amount` * if(o.`is_income`, 1, -1)) as s, c.`symbol`, k.`account_id`
	from `".KASSA_OPERATION_TABLE."` k
	join `".KASSA_OPERATION_TYPE_TABLE."` o on (o.`id` = k.`type_id`)
	join `".KASSA_CURRENCY_TABLE."` c on (c.`id` = k.`currency_id`)
	group by k.`account_id`, c.`symbol` order by s desc");
while($_row = $DB->Fetch($res))
{
	$_sum_all[$_row['account_id']][$_row['symbol']] = $_row['s'];
}
